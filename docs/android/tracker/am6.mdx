 ...
 * @param age 0 ~ 100
 * @param height unit is cm 50 ~ 255
 * @param weight unit is kg 10 ~ 300
 */
...
/**
 * @param type 0: close; 1: open
 * @param start This is the minute from the day starts, such as 10:10 am, it is 60 * 10 + 10, the range is 0 ~ 1440
 * @param end   This is the minute from the day starts, such as 10:10 am, it is 60 * 10 + 10, the range is 0 ~ 1440
 */
...
info.ts = 600;      // This is the minute from the day starts, such as 10:10 am, it is 60 * 10 + 10, the range is 0 ~ 1440
info.isOpen = true; // is open or close.
info.week = new boolean[]{true, true, true ,true, true, true, true}; //whick day is repeat, the array format is [sun, mon, tus, wed, thur, fri, sat]. true means repeat; false means don't.
/**
 * @param info
 */
    ...
    public void onDeviceNotify(String mac, String deviceType, String action, String message) {
        if (Am6Profile.Action_GetBloodOxygenData.equals(action)) {
            try {
                ...
                // Activity point data
                JSONArray pointArr = object.getJSONArray();
                for (JSONObject obj : pointArr) {
                    
                    // The start time of activity
                    String date = obj.getString(Am6Profile.GetActivityData_Date);
                    
                    JSONArray itemArr = obj.getJSONArray(Am6Profile.GetActivityData_List);

                    // A data point is generated every 10 seconds
                    for (JSONObject o : itemArr) {                       
                ...
                JSONArray reportArr = object.getJSONArray();
